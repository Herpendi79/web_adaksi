            ...
            <div class="col-md-12">
                <div class="card overflow-hidden">
                        ...
                        <div class="d-md-flex align-items-center">
                            <div>
                                <h5 class="card-title mb-1">
                                    ...
                                    <span class="text-danger">
                                        ...
                                        {{ isset($data) && isset($data->id_rk) && isset($total_masuk[$data->id_rk])
                                            ? number_format($total_masuk[$data->id_rk], 0, ',', '.')
                                            : '0' }}
                                    </span>
                                ...
                                <p class="card-title mb-1">
                                    Baju:
                                    <span class="text-danger">
                                        @if ($ukuranBajuCounts->count() > 0)
                                            @foreach ($ukuranBajuCounts as $size => $count)
                                                {{ $size }} ({{ $count }})@if (!$loop->last)
                                                    ,
                                                @endif
                                            @endforeach
                                        @else
                            ...
                            <form class="d-flex flex-stack flex-wrap gap-1 justify-content-md-end ms-auto">
                                ...
                                <a href="{{ url('admin/rakernas/pendaftar/' . $id) }}"
                                    class="btn btn-primary btn-sm d-flex align-items-center gap-1 ms-2">
                                    <i class="mdi mdi-refresh"></i> Refresh
                                    <a href="{{ route('rakernas.export_pendaftar', ['id' => $id]) }}"
                                        class="btn btn-success btn-sm d-flex align-items-center gap-1 ms-2">
                                        <i class="mdi mdi-export"></i> Export
                                    </a>

                                    <a href="{{ url('admin/rakernas/') }}"
                                        class="btn btn-info btn-sm d-flex align-items-center gap-1 ms-2">
                                ...
                                <tbody>
                                    @foreach ($pendaftar_rakernas as $key => $data)
                                        <tr>
                                            ...
                                            <td>
                                                <p class="mb-0 fw-medium fs-14">{{ $data->anggota->nama_anggota ?? '-' }}
                                                </p>
                                                <p class="mb-0 fw-medium fs-14">{{ $data->user->no_hp ?? '-' }}
                                                </p>
                                            </td>
                                            <td>
                                                <p class="mb-0 fw-medium fs-14">{{ $data->ukuran_baju ?? '-' }}</p>
                                            </td>
                                            <td>
                                                <p class="mb-0 fw-medium fs-14">{{ $data->anggota->homebase_pt ?? '-' }}
                                                </p>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                @if ($data->status == 'pending')
                                                    ...
                                                    <div class="modal fade" id="modalValidasi{{ $data->id_prk }}"
                                                        ...
                                                        <div class="modal-dialog">
                                                            <form
                                                                action="{{ url($url . '/validasiPendaftar/' . $data->id_prk) }}"
                                                                method="POST">
                                                                @csrf
                                                                <div class="modal-content">
                                                                    ...
                                                                    <div class="modal-body">
                                                                        ...
                                                                        <div class="mb-3">
                                                                            <label for="status_anggota{{ $data->id_rk }}"
                                                                                class="form-label">Bukti Pendaftaran :
                                                                            </label>
                                                                            <select class="form-select"
                                                                                id="status_anggota{{ $data->id_rk }}"
                                                                                name="valid" required>
                                                                                <option value="valid">Valid</option>
                                                                                <option value="nonvalid">Tidak Valid
                                                                                </option>
                                                                            </select>
                                                                        </div>
                                                                        <div class="mb-3">
                                                                            <label for="keterangan"
                                                                                class="form-label">Keterangan Validasi(Max
                                                                                250 karakter) Khusus untuk <strong>Tidak
                                                                                    Valid</strong> :</label>
                                                                            <input type="text" class="form-control"
                                                                                id="keterangan" name="keterangan"
                                                                                value=""
                                                                                placeholder="Wajib tuliskan keterangan untuk verifikasi yang tidak valid">
                                                                        </div>
                                                ...
                                                @else
                                                    ...
                                                    <div class="modal fade" id="modalView{{ $data->id_prk }}"
                                                        tabindex="-1" aria-labelledby="modalView{{ $data->id_prk }}"
                                                        aria-hidden="true">
                                                        <div class="modal-dialog">
                                                            <form action="" method="">
                                                                @csrf
                    ...
                    <div class="card-footer pt-3 pb-0 border-top">
                        {{ $pendaftar_rakernas->links('vendor.pagination.bootstrap-5') }}
